...
import { discoverDramas, SortOption } from "@/lib/tmdb";
...
const COUNTRY_FILTERS = [
  { label: "All", country: "", genre: "", type: "tv" as const, href: "/explore" },
  { label: "K-Drama", country: "KR", genre: "", type: "tv" as const, href: "/explore?country=KR" },
  { label: "C-Drama", country: "CN", genre: "", type: "tv" as const, href: "/explore?country=CN" },
  { label: "Hollywood", country: "US", genre: "", type: "tv" as const, href: "/explore?country=US" },
  {
    label: "Anime",
    country: "JP",
    genre: "16",
    type: "tv" as const,
    href: "/explore?country=JP&genre=16",
  },
] as const;
...
export async function generateMetadata({
  ...
  const countryLabel =
    COUNTRY_FILTERS.find((f) => f.country === country)?.label ?? "All";
  const sortLabel =
  ...
  return {
    title,
    description:
      "Browse K-dramas, C-dramas, anime, Hollywood and more on DramaFury. Filter by country, genre, and sort by popularity or air date.",
  };
}
...
export default async function ExplorePage({ searchParams }: ExplorePageProps) {
  ...
  /* Resolve effective genre — "Anime" tab forces genre 16 */
  const effectiveGenre = currentGenre || (currentCountry === "JP" ? "16" : "");
  ...
  const activeCountry =
    COUNTRY_FILTERS.find(
      (f) => f.country === currentCountry && f.genre === (currentGenre || "") && (f.type || "tv") === currentType,
    ) ?? COUNTRY_FILTERS[0];
  ...
  const sortHref = (sortParam: string) =>
    buildFilterHref("/explore", {
      ...
      type: currentType === "movie" ? "movie" : undefined,
      sort: sortParam,
      page: undefined,
    });
    ...
    <div className="max-w-[1400px] mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {COUNTRY_FILTERS.map((f) => {
          const isActive =
            f.country === currentCountry && f.genre === (currentGenre || "");
          return (
            <Link
              key={f.href}
              href={f.href}
