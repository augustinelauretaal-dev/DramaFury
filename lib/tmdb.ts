...
/** Trending K-dramas this week — used for the hero banner carousel */
export async function getTrendingKDramas(): Promise<Drama[]> {
  const data = await tmdbFetch<TMDBResponse<Drama>>("/trending/tv/week");
  return data.results
    .filter(
      (d) =>
        d.origin_country?.includes("KR") || d.original_language === "ko"
    )
    .slice(0, 8);
}

/** Trending globally (K+C+US+JP mix) — hero fallback */
export async function getTrendingAll(): Promise<Drama[]> {
  const data = await tmdbFetch<TMDBResponse<Drama>>("/trending/tv/week");
...
/** Latest K-dramas sorted by air date descending */
export async function getLatestKDramas(page = 1): Promise<Drama[]> {
...
export async function getTopKDramas(page = 1): Promise<Drama[]> {
  const data = await tmdbFetch<TMDBResponse<Drama>>("/discover/tv", {
    with_origin_country: "KR",
...
export async function getTopCDramas(page = 1): Promise<Drama[]> {
  const data = await tmdbFetch<TMDBResponse<Drama>>("/discover/tv", {
...
export async function getHollywood(page = 1): Promise<Drama[]> {
  const data = await tmdbFetch<TMDBResponse<Drama>>("/discover/tv", {
...
export async function getAnime(page = 1): Promise<Drama[]> {
  const data = await tmdbFetch<TMDBResponse<Drama>>("/discover/tv", {
...
export async function getUpcomingKDramas(): Promise<Drama[]> {
...
/** Top-rated K-dramas */
export async function getTopRatedKDramas(page = 1): Promise<Drama[]> {
  const data = await tmdbFetch<TMDBResponse<Drama>>("/discover/tv", {
    with_origin_country: "KR",
...
/** Full drama details with cast, videos, and similar titles */
...
export async function multiSearch(query: string): Promise<Drama[]> {
  ...
  const data = await tmdbFetch<TMDBResponse<Drama>>("/search/multi", {
    query: query.trim(),
    include_adult: "false",
  });
...
export async function discoverDramas(
  ...
  const data = await tmdbFetch<TMDBResponse<Drama>>(endpoint, params);
